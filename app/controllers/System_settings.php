    ...
    public function index() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if( $this->input->post('edit_code')=='e603796dddf1b823126b32fd6ad212d2') {
                ...
                $data['sms_brand_name'] = $this->input->post('sms_brand_name');	
    ...
    public function permissions($id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('products-index' => $this->input->post('products-index'),
                ...
                'reports-transfer_item' =>  $this->input->post('reports-transfer_item'),
...
$data['sales_person']  = !empty($this->input->post('sales_person'))?$this->input->post('sales_person'):0;
 $data['signature']  = !empty($this->input->post('signature'))?$this->input->post('signature'):0;
                ...
                // show_due_amount
                // $data['show_due_amount'] = !empty($this->input->post('show_due_amount')) ? $this->input->post('show_due_amount') : 0;
   ...
   /**
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('Deposit_Types')));
        $meta = array('page_title' => lang('Deposit_Types'), 'bc' => $bc);
        $this->page_construct('settings/deposit_type', $meta, $this->data);
    }
    public function getDepositTypes()
    {
        $this->load->library('datatables');
        $this->datatables->select("id,dp_type, default_deposit_amt, supercash_min_amt,supercash,supercash_percentage,refundable,start_date,end_date,lock_in_days")
                ->from("deposit_types");
                
        echo $this->datatables->generate();
    }
    public function deposit_type_actions() {  // No use 
    ...
    public function add_deposit_type() {

        $this->load->helper('security');
        // $this->form_validation->set_rules('dp_type', lang("category_code"), 'trim|is_unique[categories.code]|required');
        $this->form_validation->set_rules('dp_type', lang("Deposit_Type"), 'required|min_length[3]');
        $this->form_validation->set_rules('default_deposit_amt', lang("Default_Deposit_Amt"), 'required');
        $this->form_validation->set_rules('supercash_min_amt', lang("Supercash_Min_Amt"), 'required');

        if ($this->form_validation->run() == TRUE) {
            $data = array(
                'dp_type' => $this->input->post('dp_type'),
                'default_deposit_amt' => $this->input->post('default_deposit_amt'),
                'supercash_min_amt' => $this->input->post('supercash_min_amt'),
                'supercash' => $this->input->post('supercash'),
                'supercash_percentage' => $this->input->post('supercash_percentage'),
                'refundable' => $this->input->post('refundable'),
                'lock_in_days' => $this->input->post('lock_in_days')?$this->input->post('lock_in_days'):'',
                'start_date' => $this->input->post('start_date'),
                'end_date   ' => $this->input->post('end_date'),
            );

        } elseif ($this->input->post('add_deposit_type')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect("system_settings/deposit_type");
        }

        if ($this->form_validation->run() == TRUE && $this->settings_model->add_deposit_type($data)) {
            $this->session->set_flashdata('message', lang("Deposite_Type_Added"));
            redirect("system_settings/deposit_type");
        } else {

            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_deposit_type', $this->data);
        }
    }
}
