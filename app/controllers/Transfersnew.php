...
class Transfersnew extends MY_Controller {
    ...
    function add() {
        ...
        if ($this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_option        = isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' && $_POST['product_option'][$r] != 'undefined' && $_POST['product_option'][$r] != 'null' ? $_POST['product_option'][$r] : 0;
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                  ...
                  //  $subtotal = $this->sma->formatDecimal((($item_net_cost * $item_unit_quantity) + $pr_item_tax), 4);
                    ...
                    $batch_transfered = 1;
                    $batch_numbers[$batch_transfered] = NULL;
                    $transfer_quantity[$batch_transfered] = $item_quantity;
                    $transfer_unit_quantity[$batch_transfered] = $item_unit_quantity;
                    
                    if (!$this->Settings->batch_purchese_setting > 0 && $item_quantity > 0) {
                        
                        if ($pis = $this->site->getPurchasedItems($product_details->id, $from_warehouse_details->id, $item_option )) {
                            $tquantity   = $item_quantity;
                            $tu_quantity = $item_unit_quantity;
                            foreach ($pis as $pi) {
                                $batch_numbers[$batch_transfered] = $pi->batch_number;
                                if ($pi->quantity_balance >= $tquantity && $tquantity > 0) {                                     
                                    $transfer_quantity[$batch_transfered] = $tquantity;
                                    $transfer_unit_quantity[$batch_transfered] = $tu_quantity;
                                    $tquantity = 0;
                                } elseif ($tquantity > 0) {
                                    $transfer_quantity[$batch_transfered] = $pi->quantity_balance;
                                    $transfer_unit_quantity[$batch_transfered] = ($tquantity == $tu_quantity) ? $pi->quantity_balance : $pi->unit_quantity;
                                    $tquantity -= $pi->quantity_balance;
                                    $tu_quantity -= ($tquantity == $tu_quantity) ? $pi->quantity_balance : $pi->unit_quantity;
                                }                                 
                    ...
                    for($ti=1 ; $ti <= $batch_transfered ; $ti++) {
                        
                        $pr_item_tax = $this->sma->formatDecimal($item_tax * $transfer_unit_quantity[$ti], 4);
                        $product_tax += $pr_item_tax;
                        $subtotal = $this->sma->formatDecimal((($item_net_cost * $transfer_unit_quantity[$ti]) + $pr_item_tax), 4);
                        ...
                        $products[] = [
                            ...
                            'batch_number'      => $batch_numbers[$ti],
                            'net_unit_cost'     => $item_net_cost,
                            'unit_cost'         => $this->sma->formatDecimal($item_net_cost + $item_tax, 4),
                            'product_unit_id'   => $item_unit,
                            'product_unit_code' => $unit->code,
                            'quantity'          => $transfer_quantity[$ti],
                            'unit_quantity'     => $transfer_unit_quantity[$ti],
                            'quantity_balance'  => $transfer_quantity[$ti],
    ...
    function edit($id = NULL) {
        ...
        if (empty($transfer)) {
            $this->session->set_flashdata('error', lang('Transfer not found'));
            redirect("transfersnew");
        }
...
//                $row->warehousestock =$get_qty->quantity; 
                ...
                if ($this->data['transfer']->status == 'partial_completed') {
                    ...
                    $item->batch_number = $item->batch_number ? $item->batch_number : NULL;

                    $Res = $this->transfers_model->getPurchaseItemByID($id, $item->product_id, $item->option_id, $item->batch_number);
...
//                $row->quantity += $item->quantity_balance;
                ...
                $row->batch_number = $item->batch_number ? $item->batch_number : '';
                ...
                $pis = $this->site->getPurchasedItems($item->product_id, $item->warehouse_id, $item->option_id, $item->batch_number);
...
//                }
                ...
                $row->stockwarehouse2 = $this->getstockwarehousedata($this->data['transfer']->to_warehouse_id, $row->id, ($item->option_id) ? $item->option_id : NULL, $item->batch_number);
                ...
                $ri = $this->Settings->item_addition ? $ri : $c;
                 
                $pr[$ri] = array('id' => $IdRow, 'item_id' => $row->id, 'option_id' => $row->option, 'batch_number' => $row->batch_number , 'label' => $row->name . " (" . $row->code . ")",
    ...
    function delete($id = NULL) {
        ...
        $DatalogArr = array('data' => $transfer, 'products' => $transfer_items);
        $DataLog = array(
            'action_type' => 'Delete',
            'product_id' => '',
            'quantity' => '',
            'action_reff_id' => $id,
            'action_affected_data' => json_encode($DatalogArr),
            'action_comment' => 'Delete transfers',
        );
    ...
    function suggestions() {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
               ...
               // $options = $this->transfers_model->getProductOptions($row->id, $warehouse_id, $check_zero);
                if($row->storage_type == 'packed') {
                    $options = $this->products_model->getProductOptions($row->id);
                if ($options) {
    ...
    function update_status($id) {
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->updateStatus($id, $status, $note)) {
            $DatalogArr = array('status' => $status, 'note' => $note, 'type' => 'Update Status');
            $DataLog = array(
                'action_type' => 'Edit',
                'product_id' => '',
                'quantity' => '',
    ...
    public function getstockwarehousedata($warehouse, $productid, $option, $batch_number=null) {

        $where_clause = ['product_id' => $productid, 'warehouse_id' => $warehouse];
        $select = 'quantity as werehouse_2_quantity';
        $from = 'warehouses_products';
        $where_status = '';

        if ($option) {            
            $where_clause['option_id'] = $option; 
            $from = 'warehouses_products_variants';             
        }

        if($batch_number) {
            $select = 'sum(quantity_balance) as werehouse_2_quantity';
            $from = 'purchase_items'; 
            $where_clause['batch_number'] = $batch_number;
            $where_status = " status IN ('received','partial')"; 
             
        }
            $this->db->select($select);
            $this->db->where($where_clause);
        if($where_status) { $this->db->where($where_status); }
        $sql = $this->db->get($from)->row();
