...
class Pos extends MY_Controller {
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        ...
                        else if ($_POST['paid_by'][$r] == 'supercash') {
                            // die("supercash");
                            if (!$this->site->check_customer_deposit($customer_id, $amount)) {
                                $this->session->set_flashdata('error', lang("amount_greater_than_supercash"));
                                if ($_POST['submit_type'] == 'notprint') {
                                    exit;
                                } else {
                                    redirect($_SERVER["HTTP_REFERER"]);
                                ...
                                $supercash = $this->site->customerSupercashAmt($customer_id);
                                $supercash_amt = $supercash - $amount;
                                $payment[] = array(
                                    'date' => $date,
                                    // 'reference_no' => $this->site->getReference('pay'),
                                    'amount' =>$amount,
                                ...
                                $supercashLog[] = [
                                    ...
                                    "amount" => $amount,
                                    "cr_dr" => 'DR',
                                    "opening_balance" => ((bool) $supercash ? $supercash : 0),
                                    "closing_balance" => $supercash_amt,
                                    "created_by" => $this->session->userdata('user_id'),
                                ];
                            }
                        }
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    if (count($supercashLog)) {
                        $this->load->model('companies_model');
                        foreach ($supercashLog as $supercash_Log) {
                            $supercash_Log["transaction_details"] = json_encode(["table_name" => "sma_payments", "where" => ["sale_id" => $sale['sale_id'], "paid_by" => "supercash"]]);

                            $this->companies_model->set_customer_supercash_wallet_log($supercash_Log);
                        }
                    }
    ...
    public function close_register($user_id = null) {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['supercash'] = $this->pos_model->getSupercashCloseRegister($register_open_time, $user_id);
            $this->data['depositsales'] = $this->pos_model->getDepCloseRegister($register_open_time, $user_id);
            /* --- 13-03-19 --- */
...
//        }//end if.
    ...
    public function register_details() {
        ...
        $this->data['supercash'] = $this->pos_model->getRegisterSupSales($register_open_time);
        /* end */
        /* --- 13-03-19 --- */
        $this->data['othersales'] = $this->pos_model->getRegisterPaymentSales($register_open_time, $user_id, 'other');
        $this->data['dcsales'] = $this->pos_model->getRegisterPaymentSales($register_open_time, $user_id, 'DC');
    ...
    public function today_sale() {
        ...
        $this->data['supercash'] = $this->pos_model->getTodaySupercash($user_id);//supercash
        /* 5-11-2019 */
    ...
    public function settings() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'supercash_amount' => $this->input->post('Super_Cash'),
...
// redirect("pos/view/$sale_id");
    ...
    // supercash finalize sale screen
    public function searchSupercashByCustomer($customer_id = null, $bill_amt = null) {
        if ($this->input->get('customer_id')) {
            $customer_id = $this->input->get('customer_id', true);
        }
        if ($this->input->get('bill_amt')) {
        ...
        $data = $this->pos_model->searchSupercashByCustomer($customer_id);
        echo json_encode($data);
    }
    ...
    public function send_sms($code, $no) {
        ...
        $str = '';
        if ($inv->grand_total):
            $str = 'Thanks for visiting ' . $this->Settings->site_name . '. Your Total Bill amount ' . $this->sma->formatDecimal($inv->grand_total + $inv->rounding) . ' ' . $this->Settings->default_currency;
        endif;
...
//                $html .='</tr>';
    ...
    public function getTax()
    {
        ...
        $mo = $this->input->get('tax');
        if($mo != 1)
        {
            $this->session->set_userdata('pro_tax',$mo);
        }
        ...
        $response = [
            ...
            // 'seats' => "hello surya",
            // 'msg' => 'Success',
            'tax' => $this->session->userdata('pro_tax')?$this->session->userdata('pro_tax'):1

        ];
        // echo "session<br>";
        // echo $this->session->userdata('pro_tax');
        echo json_encode($response);
    }
    public function getSuperCash()
    {
        $mo = $this->input->get('mobile_no');
        $result = $this->companies_model->getSuperCash($mo);
        echo json_encode($result);
        // $response = array();
