...
class Customers extends MY_Controller {
    ...
    public function getCustomers() {
        ...
        {          
                 $this->datatables
                ->select("id, company, name, email, phone, price_group_name, customer_group_name, gstn_no, deposit_amount,award_points, cf1,cf2,supercash_amount,(deposit_amount + supercash_amount) as total_balance");
    ...
    public function add_deposit($company_id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'dp_type_id' => $this->input->post('deposit_type'),
            ...
            $super_cash = $this->input->post('super_price') ? $this->input->post('super_price') : NULL;
            if( $pos->supercash_amount==1)
            {
                if($super_cash != NULL)// true
                {
                    $cdata = array(
                        'deposit_amount' => ($company->deposit_amount + ($this->input->post('amount') - $super_cash)),
                        // 'supercash_amount' => ($this->input->post('super_price'))
                         'supercash_amount' => ($this->input->post('super_price')+$company->supercash_amount)
                    ); 
                }
                else //false
                {
            ...
            }else
            {
                $cdata = array(
                    ...
                    'supercash_amount' =>$company->supercash_amount
                );
            }
            if( $pos->supercash_amount==1)
            {
                $depositLog1 = [
                    ...
                    "amount" => $super_cash,
                    "cr_dr" => 'CR',
                    "opening_balance" => ((bool) $company->supercash_amount ? $company->supercash_amount : 0),
                    "closing_balance" => ((float) $company->supercash_amount + $super_cash),
                    "created_by" => $this->session->userdata('user_id'),
                ];
                $depositLog = [
                    "customer_id" => $company->id,
                    "date" => $date,
                    "descriptions" => "Add Amount",
                    "amount" => ($this->input->post('amount') - $super_cash),
                    "cr_dr" => 'CR',
                    "opening_balance" => ((bool) $company->deposit_amount ? $company->deposit_amount : 0),
                    "closing_balance" => ((float) $company->deposit_amount + (float) $this->input->post('amount') - $super_cash),
                    "created_by" => $this->session->userdata('user_id'),
                ];
            }else{
                $depositLog = [
        ...
        if ($this->form_validation->run() == true && $deposit_id = $this->companies_model->addDeposit($data, $cdata)) {
            ...
            if( $pos->supercash_amount==1)
            {
                $depositLog1["transaction_details"] = json_encode(["table_name" => "sma_deposits", "where" => ["id" => $deposit_id]]);
                $this->companies_model->set_customer_supercash_wallet_log($depositLog1);
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['company'] = $company;
            $this->data['depositTypes'] = $this->companies_model->getDepositTypes();
     ...
     */
    public function supercashAmt($customerId) {
        $result = $this->companies_model->getsupercashAmt($customerId);
        echo json_encode($result);
    //ShowTotalDeposit
    } 
    // public function totaldeposit($customerId) {
    //     $result = $this->companies_model->totaldeposit($customerId);
    //     echo json_encode($result);
    // }
...
//             ->setLocked(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
            ...
            if (isset($_FILES["deposit_file"])) /* if($_FILES['userfile']['size'] > 0) */ {
                ...
                // $csv = $this->upload->file_name;
                // b2d136499a83fd694b18ea23981bc814
                // echo "b2d136499a83fd694b18ea23981bc814<br>";
                // echo $csv;
                // exit;
                ...
                // print_r($titles);
                // exit;
                // $keys = array('name', 'phone', 'customer_group_name', 'address', 'city', 'state', 'state_code', 'postal_code', 'country', 'gstn_no', 'vat_no', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6', 'deposit_amount');
                ...
                // foreach ($final as $record) {
                //     if ($record['name'] != '' && $record['phone'] != '' && $record['customer_group_name'] != '' && $record['cf1'] != ''&& $record['cf2'] != ''&& $record['deposit_amount'] != '') {
                //         $record['group_id'] = 3;
                //         $record['group_name'] = 'customer';
                //         $record['customer_group_id'] = 1;
                //         $record['customer_group_name'] = 'General';
    ...
    public function getSuperCashPercentage()
    {

        $id = $this->input->get('id');

        $supercash_percentage = $this->db->select('supercash_percentage')->where(['id' => $id])->get('deposit_types')->row();
        $reponse = [
            'supercash_percentage' => ($supercash_percentage->supercash_percentage) ? $supercash_percentage->supercash_percentage : '',
        ];
