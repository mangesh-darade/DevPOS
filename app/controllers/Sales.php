    ...
    public function return_sale($id = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->input->post('amount-paid') && $this->input->post('amount-paid') > 0) {
                ...
                $user = $this->session->userdata('user_id');
                if ($this->input->post('paid_by') === 'deposit') {
                    $total = $this->sma->formatDecimal($this->input->post('amount-paid'), 4);
                    $payment = array(
                        'date' => $date,
                ...
                } else if ($this->input->post('paid_by') == 'supercash') {
                //    echo "sup[ercash";
                //    exit;
                    $payment = array(
                        'date' => $date,
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return, $extrasPara,$user)) {
            ...
            foreach ($inv_items as $item) {
                ...
                if ($UnitQty != 0) {
                    ...
                    // $row->unit_price        = $row->base_unit_price;//$unit_price;
                    // $row->net_unit_price    = $this->sma->formatDecimal($row->unit_price - ($item->item_tax / $item->quantity));
                    $row->real_unit_price   = ((int)$item->real_unit_price > 0) ? $item->real_unit_price : $row->net_unit_price;
                    // print_r($net_unit_price);
                    // exit;
                    $row->tax_rate          = $item->tax_rate_id;
                    $row->serial            = $item->serial_no;
                    $row->option            = $item->option_id;
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                elseif ($this->input->post('form_action') == 'export_to_json') {
                    ...
                    force_download('./Json_data.json', NULL);
                    // exit;
