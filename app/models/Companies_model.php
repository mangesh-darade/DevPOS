...
class Companies_model extends CI_Model {
    ...
    //get supercash for finalize screen
    
    public function getsupercashAmt($customerId) {
        $supercash = $this->db->select('supercash_amount,deposit_amount')->where(['id' => $customerId])->get('companies')->row();
        $reponse = [
            'supercash' => ($supercash->supercash_amount) ? $this->sma->formatMoney(round($supercash->supercash_amount, 2)) : '',
            'deposit_amount' => ($supercash->deposit_amount) ? $this->sma->formatMoney(round(($supercash->deposit_amount+$supercash->supercash_amount), 2)) : '',
        ];
        return $reponse;
    }
     //ShowTotalDeposit
    //  public function totaldeposit($customerId) {
    //     $supercash = $this->db->select ('supercash_amount','deposit_amount')->where(['id' => $customerId])->get('companies')->row();
    //     $reponse = [
    //         'supercash' => ($supercash->supercash_amount) ? round($supercash->supercash_amount, 2) : '',
    //         'deposit' => ($supercash->deposit_amount) ? round($supercash->deposit_amount, 2) : '',
    //     ];
    //     return $reponse;
    // }
    ...
    public function getDepositandGift($customerId) {
        ...
        $reponse = [
            'deposit' => ($deposit->deposit_amount) ? $this->sma->formatMoney(round($deposit->deposit_amount, 2)) : '',
            'deposit_amt' => $deposit->deposit_amount,
    ...
    public function set_customer_supercash_wallet_log(array $logData) {

        if (is_array($logData)) {

            $this->db->insert("customer_supercash_wallet", $logData);
    ...
    // {
    // $quary= "select * from sma_companies";
    // // $quary= "select name,phone,customer_group_name,cf1,cf2,deposit_amount from sma_companies";
	...
	public function importBulkDeposit($data = array(),$user)
	 {
        ...
        $pos=$this->data['pos'] = $this->pos_model->getSetting();
        if (!empty($data)) {
            $i=0;
            foreach ($data as $itesms) {
                // echo "<br>";
                ...
                $quary = "select sma_companies.id,deposit_amount,supercash_amount from sma_companies where phone=".$itesms['phone'];
                $res= $this->db->query($quary);
                // echo $su = $res->result()[0]->supercash_amount;
                // echo $id = $res->result()[0]->id;
                // exit;
                $id = $res->result()[0]->id;
                ...
                $new_dep2 =  $deposit + $data[$i]['deposit_amount'];
            ...
            if( $pos->supercash_amount==1)
            {
                if($su_cash != 0)// true
                {
                    $this->db->update('companies', array('deposit_amount' => $new_dep2,'supercash_amount' => ($res->result()[0]->supercash_amount + $su_cash)), array('id' => $id));
                }
                else //false
                {
                    $this->db->update('companies', array('deposit_amount' => $new_dep2), array('id' => $id));
                }
                ...
                $deposit_arr = [
                 ...
                 if($res)
                 {
                  $deposit = $deposit + $res->result()[0]->supercash_amount;
                  $new_dep = $deposit + $amount;
            ...
            }else
            {
                ...
                {
                    // // INSERT INTO `sma_deposits` (`id`, `date`, `company_id`, `amount`, `paid_by`, `note`, `created_by`, `updated_by`, `updated_at`, `super_cash`, `services`) VALUES (NULL, CURRENT_TIMESTAMP, '147', '100.00', 'cash', 'services', '1', '1', NULL, '50', '0');
                    // $sql = "INSERT INTO `sma_deposits` (`id`, `date`, `company_id`, `amount`, `paid_by`, `note`, `created_by`, `updated_by`, `updated_at`, `super_cash`, `services`) VALUES (NULL, CURRENT_TIMESTAMP, $id, $amount,'".$de_type."', 'services',$user  , 0, NULL,".$su_cash.", '0')";
                    // // $this->db->insert('deposits', $deposit);
    ...
    public function getSuperCash($phone) {
        $supercash = $this->db->select('supercash_amount')->where(['phone' => $phone])->get('companies')->row();
        $reponse = [
            'supercash' => ($supercash->supercash_amount) ? round($supercash->supercash_amount, 2) : 0,
        ];
        return $reponse;
    }
    public function getDepositTypes()
    {
        $q = $this->db->get('deposit_types');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
