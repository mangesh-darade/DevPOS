...
//                    . "(IF({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', {$this->db->dbprefix('products')}.name LIKE '%" . $term . "%',Replace(coalesce({$this->db->dbprefix('products')}.name,''), ' ','') LIKE '%" . str_replace(" ", "", $term) . "%') OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')");
    ...
    public function addSale($data = [], $items = [], $payments = [], $si_return = [], $extrasPara = [],$user) {
                    ...
                    if ($payments['paid_by'] == 'gift_card') {
                        ...
                        if ($payments['paid_by'] == 'deposit') {
                            $customer = $this->site->getCompanyByID($data['customer_id']);
                             ...
                             $opening =$customer->deposit_amount;
                             $new_dep = $customer->deposit_amount + ($payments['amount']);
                             $newId = $customer->id;
                             ...
                             $sqlnew = "INSERT INTO `sma_customer_wallet_transactions`(`id`, `customer_id`, `date`, `descriptions`, `amount`, `cr_dr`, `transaction_details`, `opening_balance`, `closing_balance`, `updated_at`, `created_by`) VALUES (NULL,$customer->id,CURRENT_TIMESTAMP,'Add Amount',$deposit,'CR',$str,$opening,$new_dep,CURRENT_TIMESTAMP,$user)";
                        ...
                        } else if ($payments['paid_by'] == 'supercash') {
                            $customer = $this->site->getCompanyByID($data['customer_id']);
                            $deposit = - $payments['amount'];
                            $opening =$customer->supercash_amount;
                            $new_dep = $customer->supercash_amount - ($payments['amount']);
                            $newId = $customer->id;
                           
                          
                            // $this->db->update('companies', array('deposit_amount' => $payments['cc_holder']), array('id' => $data['customer_id']));
                            $this->db->update('companies', array('supercash_amount' => (($customer->supercash_amount)-($payments['amount']))), array('id' => $customer->id));
                            $str = "'".'{"table_name":"sma_deposits","where":{"id": '.$newId.'}}'."'";
                            $sqlnew = "INSERT INTO `sma_customer_supercash_wallet`(`id`, `customer_id`, `date`, `descriptions`, `amount`, `cr_dr`, `transaction_details`, `opening_balance`, `closing_balance`, `updated_at`, `created_by`) VALUES (NULL,$customer->id,CURRENT_TIMESTAMP,'Add Amount',$deposit,'CR',$str,$opening,$new_dep,CURRENT_TIMESTAMP,$user)";
                            $this->db->query($sqlnew);
 ...
 // Urbanpiper Stock Manage 
    ...
    */
    ...
    {
        // if($data['note'] == "Sale Return")
        // {
        //     $sql = "SELECT date,closing_balance FROM `sma_customer_supercash_wallet` where  customer_id = ".$data['company_id']." order by date desc ";
