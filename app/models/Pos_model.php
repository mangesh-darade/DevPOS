...
class Pos_model extends CI_Model {
    ...
    public function getRegisterData($rid= null, $user_id) {
        if (!$user_id) {
            $user_id = $this->session->userdata('user_id');
        }
        $q = $this->db->get_where('pos_register', array('id' =>$rid,'user_id' => $user_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
...
//                    echo '<pre>';
                ...
                foreach ($payments as $payment) {
                    if (!empty($payment) && isset($payment['amount']) && $payment['amount'] != 0) {
                        ...
                        if ($payment['paid_by'] == 'ppp') {
                            ...
                            elseif ($payment['paid_by'] == 'supercash') {
                                $customer = $this->site->getCompanyByID($data['customer_id']);
                                $this->db->update('companies', array('supercash_amount' => ($customer->supercash_amount - $payment['amount'])), array('id' => $customer->id));
                            }
                            unset($payment['cc_cvv2']);
 ...
 $printerSettings = $this->site->defaultPrinterOption($this->Settings->default_printer);
    ...
    // supercash on Todays Sale
    public function getTodaySupercash($user_id=NULL) {
        // $date = date('Y-m-d');
        // $this->db->select('SUM( COALESCE( amount, 0 ) ) AS total', FALSE)
        //         ->where('date like ','%'.$date.'%')
        //         ->where('paid_by','supercash');
        // if($user_id) {
        //     $this->db->where('created_by', $user_id);
        // }
        // $q = $this->db->get('payments');
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_gc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >', $date)->where('payments.paid_by', 'supercash');
        if($user_id) {
            $this->db->where('payments.created_by', $user_id);
        }
        $q = $this->db->get('payments');
    ...
    public function getClosedRegisterSales($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate);
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getCloseddueAmt($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS duetotal', FALSE)
                ->where('payment_status', 'due')->where('date >=', $sdate)->where('date <=', $edate);
        $this->db->where('created_by', $user_id);
    ...
    public function getClosedpartialAmt($sdate,$edate,$user_id = NULL) {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) )  - SUM( COALESCE( paid, 0 ) ) AS partial_due', FALSE)
                ->where('payment_status', 'partial')->where('date >=', $sdate)->where('date <=', $edate);
        $this->db->where('created_by', $user_id);
    ...
    public function getClosedRegisterCCSales($sdate,$edate, $user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('payments.paid_by', 'CC');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterCashSales($sdate,$edate, $user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                // ->where('type', 'received')->where('payments.date >', $date)->where('payments.paid_by', 'cash');
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('payments.paid_by', 'cash');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterRefunds($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS returned', FALSE)
                ->join('sales', 'sales.id=payments.return_id', 'left')
                ->where('type', 'returned')->where('payments.date >=', $sdate)->where('payments.date <=', $edate);
        $this->db->where('payments.created_by', $user_id);
        $this->db->where('payments.paid_by ', 'cash');
        $q = $this->db->get('payments');
        if ($q->num_rows() > 0) {
    ...
    public function getClosedRegisterCashRefunds($sdate,$edate, $user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS returned', FALSE)
                ->join('sales', 'sales.id=payments.return_id', 'left')
                ->where('type', 'returned')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('payments.paid_by', 'cash');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterExpenses($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
    ...
    public function getClosedRegisterChSales($sdate,$edate, $user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('payments.paid_by', 'Cheque');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterGCSales($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'gift_card');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterPPPSales($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'ppp');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getRegisterSupSales($date, $user_id = NULL) {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >', $date)->where('paid_by', 'supercash');
        $this->db->where('payments.created_by', $user_id);
    ...
    //supercsh close register
    public function getSupercashCloseRegister($date, $user_id = NULL) {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        if (!$user_id) {
    ...
    public function getClosedSupercashRegister($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'supercash');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getDepCloseRegister($date, $user_id = NULL) {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        if (!$user_id) {
    ...
    public function getClosedDepositRegister($sdate,$edate,$user_id = NULL) {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'deposit');
        $this->db->where('payments.created_by', $user_id);
    ...
    /* -- End 13-03-19 --- */
    public function getClosedRegisterPaymentSales($sdate,$edate,$user_id = NULL, $condition) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', $condition);
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterStripeSales($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'stripe');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getClosedRegisterAuthorizeSales($sdate,$edate,$user_id = NULL) {
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
        if (!$user_id) {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >=', $sdate)->where('payments.date <=', $edate)->where('paid_by', 'authorize');
        $this->db->where('payments.created_by', $user_id);
...
//                exit;
    ...
    /* 06-11-2019 */
    //supercash finalize sale
    public function searchSupercashByCustomer($customer_id) {
        $result = $this->db->query("SELECT c.supercash_amount as balanceamt ,d.amount as value FROM sma_companies as c left join  sma_deposits d on d.company_id=c.id  WHERE  c.id = '$customer_id' ");
...
//                $this->db->group_by('date(date)');
    ...
    public function getClosedRegisterDeposit($register_open_time,$register_close_time, $user_id = NULL){
        // if (!$date) {
        //     $date = $this->session->userdata('register_open_time');
        // }
         ...
         $this->db->select("sum( COALESCE(amount, 0)) as deposit_amount,  group_concat(DISTINCT  paid_by SEPARATOR ',') as paid_by")
                ->where('DATE_FORMAT(date,"%Y-%m-%d") >=',$register_open_time)->where('DATE_FORMAT(date,"%Y-%m-%d") <=',$register_close_time);
                $this->db->where('created_by', $user_id);
//                $this->db->group_by('date(date)');
               $getData =    $this->db->get('deposits')->row();
