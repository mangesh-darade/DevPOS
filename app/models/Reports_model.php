...
//26-09-2020
    ...
    public function getCustomerSupercashLedger($customerId = null, $startDate = null, $enddate = null) {
        ...
        if ($startDate) {
            ...
            $sql = "SELECT date,closing_balance FROM `sma_customer_wallet_transactions` where  date like '%".$d[0]."%' order by date desc ";

            $closing_balance = $this->db->query($sql)->result()[0]->closing_balance;
        }
        ...
        if($startDate)
        {
            $wallet_deposit_closing =  $closing_balance;
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach (($q->result()) as $key => $row) {
                ...
                { 
                    $wallet_deposit_opening_flag =  $row->wallet_deposit_opening;
                    
                    $wallet_supercash_opening= $supercahs_wallet_amt['wallet_supercash_opening'];
                    $wallet_supercash_closing= $supercahs_wallet_amt['wallet_supercash_closing'];
                    $wallet_spend_closing= $total_spend['supercash_spend'];
                ...
                {
                    // exit;
                    $wallet_deposit_closing =  $row->wallet_deposit_closing;
                    $wallet_deposit_opening =  0; //$row->wallet_deposit_opening;
                    $wallet_supercash_opening= $supercahs_wallet_amt['wallet_supercash_opening'];                    
                    $wallet_supercash_closing= $supercahs_wallet_amt['wallet_supercash_closing']; 
                ...
                $data[$row->customer_id] = [
                    ...
                    "wallet_deposit_opening" =>$wallet_deposit_opening_flag ? $wallet_deposit_opening_flag : $wallet_deposit_opening,//$row->wallet_deposit_opening,  // cwt F
                    "wallet_supercash_opening" => $wallet_supercash_opening,//$supercahs_wallet_amt->wallet_supercash_opening, //cswt G

                    // "recharge"=>($total_recharge->recharge - $total_recharge->total_supercash_recharge), // dpt I
                    ...
                    "wallet_deposit_closing" => $wallet_deposit_closing,  // cwt O
                    "wallet_supercash_closing" => $wallet_supercash_closing,//$supercahs_wallet_amt->wallet_supercash_closing, //cswt P
    ...
    public function getTotalDepositeById($customerId = null, $startDate = null, $enddate = null)
    {
        ...
        $this->db->where('dp.note !=', 'Sale Return');
    ...
    public function getSupercashWalletAmtById($customerId = null, $startDate = null, $enddate = null)
    {
        ...
        if ($enddate) {
            $d = explode(" ",$enddate);
            // echo $d[0];
            $sql = "SELECT date,closing_balance FROM `sma_customer_supercash_wallet` where  date like '%".$d[0]."%' order by date desc ";
        ...
        $size = sizeof($q->result());
        if($startDate)
            {
  
                $row = [
                    'date' => $q->result()[0]->date,
                    'wallet_supercash_opening' => $q->result()[$size-1]->wallet_supercash_opening,
                    // 'wallet_supercash_closing' => $q->result()[0]->wallet_supercash_closing,
                    'wallet_supercash_closing' => $closing_balance
                ];
    ...
    public function getTotalSpendAmtById($customerId = null, $startDate = null, $enddate = null)
    {
        $spend = [];
        $total_deposite=0;
        $total_return_deposite=0;

        $total_supercash=0;
        $total_return_supercash=0;

        // $customerId = 254;
        $this->db->select("DATE(pmt.date) date, SUM(pmt.amount) as total_deposite");
        ...
        $array = array('pmt.paid_by' => "deposit", 'pmt.type' => "received");

        $this->db->where($array); 

         $q = $this->db->get();//->result();
         $total_deposite = $q->result()[0]->total_deposite;


        //  ----------------------------------------
         $this->db->select("DATE(pmt.date) date, SUM(pmt.amount) as total_return_deposite");
            $this->db->from('payments pmt');
         $this->db->join('sales sale', 'sale.id = pmt.sale_id', 'left');
        ...
        $array = array('pmt.paid_by' => "deposit", 'pmt.type' => "returned");

        $this->db->where($array); 

         $q = $this->db->get();//->result();

         $total_return_deposite = $q->result()[0]->total_return_deposite;


         $spend['deposite_spend']= $total_deposite -  $total_return_deposite;
        ...
        $this->db->select("DATE(pmt.date) date, SUM(pmt.amount) as total_supercash");
        $this->db->from('payments pmt');
         $this->db->join('sales sale', 'sale.id = pmt.sale_id', 'left');
        ...
        $array = array('pmt.paid_by' => "supercash", 'pmt.type' => "received");

        $this->db->where($array);

        $q = $this->db->get();//->result();
 
        $total_supercash= $q->result()[0]->total_supercash; 
        // -------------------
        $this->db->select("DATE(pmt.date) date, SUM(pmt.amount) as total_return_supercash");
        $this->db->from('payments pmt');
         $this->db->join('sales sale', 'sale.id = pmt.sale_id', 'left');
        ...
        $array = array('pmt.paid_by' => "supercash", 'pmt.type' => "returned");

        $this->db->where($array);
        $q = $this->db->get();//->result();
        $total_return_supercash= $q->result()[0]->total_return_supercash;

        $spend['supercash_spend']= $total_supercash + $total_return_supercash;
    ...
    public function Deposit_opening_from_wallet()
    {

    $this->db->select("cwt.amount");
    $this->db->from('customer_wallet_transactions cwt');
    $this->db->where('cwt.cr_dr= "CR"');
    $this->db->join('companies co', 'cwt.customer_id = co.id', 'left');
    $q = $this->db->get()->result();
    return $q;
