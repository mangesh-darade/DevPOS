...
<script>
    $(document).ready(function () {
        ...
        var oTable = $('#PrRData').dataTable({
            ...
            "aoColumns": [{"mRender": fsd} , null, null, null, null,null, null, null, null, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, null, {"mRender": currencyFormat}, {"mRender": currencyFormat}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
					if(aaData[aiDisplay[i]][9]!=null || aaData[aiDisplay[i]][9]==''){
                                            purcost +=  parseFloat(aaData[aiDisplay[i]][9]);
                                        }
                                   
                                        if(aaData[aiDisplay[i]][10]!=null || aaData[aiDisplay[i]][10]==''){
					sprice += parseFloat(aaData[aiDisplay[i]][10]);
                                        } 
                                        if(aaData[aiDisplay[i]][11]!=null || aaData[aiDisplay[i]][11]==''){
                                        sdis += parseFloat(aaData[aiDisplay[i]][11]);
                                        }
                                        if(aaData[aiDisplay[i]][12]!=null || aaData[aiDisplay[i]][12]==''){
					gamt += parseFloat(aaData[aiDisplay[i]][12]);
                                        }
                                        if(aaData[aiDisplay[i]][14]!=null || aaData[aiDisplay[i]][14]==''){
					grpft += parseFloat(aaData[aiDisplay[i]][14]);
                                        }
                                                    
                                        if(aaData[aiDisplay[i]][15]!=null || aaData[aiDisplay[i]][15]==''){
					ntpft += parseFloat(aaData[aiDisplay[i]][15]);
                                        }
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[9].innerHTML = '<div class="text-right">' + formatMoney(purcost) + '</div>';
                nCells[10].innerHTML = '<div class="text-right">' + formatMoney(sprice) + '</div>';
                nCells[11].innerHTML = '<div class="text-right">' + formatMoney(sdis) + '</div>';
                nCells[12].innerHTML = '<div class="text-right">' + formatMoney(gamt) + '</div>';
                nCells[14].innerHTML = '<div class="text-right">' + formatMoney(grpft) + '</div>';
                nCells[15].innerHTML = '<div class="text-right">' + formatMoney(ntpft) + '</div>';
