...
//echo form_dropdown('table_id', $table_id, $tables);
                        ...
                        if (isset($Settings->pos_type) && $Settings->pos_type == 'restaurant') {
                            ...
                            <div class="radio-btn-section">
                                                        ...
                                                        if(in_array($rtval->id, $tableselected)){
                                                                 ...
                                                                 echo '<div class="btn-group">';
                                                                    ...
                                                                    echo '<div class="dropdown-menu" style="padding: 10px 20px; color:#000">';
                                                                      echo '<ul class="sub_table">';
                                                                        ...
                                                                        echo '<li onclick="add_subtable(\'B\','.$rtval->id.')" class="dropdown-item" >B</li>';
                                                                        echo '<li onclick="add_subtable(\'C\','.$rtval->id.')" class="dropdown-item" >C</li>';
                                                                        echo '<li onclick="add_subtable(\'D\','.$rtval->id.')" class="dropdown-item" >D</li>';
                                                                      echo '</ul>';
                                                                   echo ' </div>';
                                                                  echo '</div>'; 
                                                          ...
                                                          echo '</div>';
                                                            ...
                                                            foreach (range('B', 'D') as $char) {
                                                              $style_lablesub ='';
                                                              $call_id_sub ='';
                                                              if (($table_id != $rtval->id) && ($rtval->name.'-'.$char == $rtval->suspended_note)) {
                                                                    //var_dump($rtval->status);
                                                                    $style_lablesub = ($rtval->status == 'Booked') ? "background:red" : "";
                                                                    $call_id_sub    = $rtval->suspended_id;
                                                                   //$style_desable = ($rtval->status=='Booked')? "disabled" :"";
                                                               }
                                                              
                                                               $checksub = ($rtval->name.'-'.$char == $rtval->suspended_note)? true :false; 
                                                               if($suspend_sale->suspend_note == $rtval->name.'-'.$char){
                                                                   echo '<div class="col-sm-3 table-block"  id="table_block-'.$char.'-'.$rtval->id.'">';
                                                               }else{
                                                                   echo '<div class="col-sm-3 table-block" '.((($checksub == false) )?' style="display:none;"':'' ).' id="table_block-'.$char.'-'.$rtval->id.'">';
                                                               }
                                                                echo '<div id="select_table-'.$rtval->id.'" class="rd-btn resturent_table_group" '.(($call_id)? 'onclick=callTable("'.$call_id.'")' : '').' >';
                                                                       echo '<input ' . $checked . ' type="radio"  table_id="' . $rtval->id . '"  name="gender" id="' . str_replace(" ", "_", $rtval->name.'-'.$char) . '" value="' . $rtval->name.'-'.$char. '" ' . $style_desable . '>';
                                                                       echo '<lable style="' . $style_lablesub . '">' . $rtval->name.'-'.$char . '</lable>';
                                                                echo '</div>';
                                                                 echo '<div style="display:block; padding: 8px;">';
                                                                   echo '<button type="button" onclick="delete_subtable(\''.$char.'\','.$rtval->id.')" class=" btn btn-xs btn-danger">Delete</button> ';
                                ...
                                <?php
                               /* foreach($tables_groups as $items_tableG ){
                                   echo '<strong style="display: block;clear: both; color: #FF0000; padding:10px">'. $items_tableG['table_group'].'</strong>';
                                foreach ($tables as $rtkey => $rtval) {
                                    ...
                                    echo '<div class="rd-btn resturent_table_group" '.(isset($call_id)? 'onclick=callTable("'.$call_id.'")' : '').' >';
                                    echo '<input ' . $checked . ' type="radio"  table_id="' . $rtval->id . '"  name="gender" id="' . str_replace(" ", "_", $rtval->name) . '" value="' . $rtval->name . '" ' . $style_desable . '>';
                                    echo '<lable style="' . $style_lable . '">' . $rtval->name . '</lable>';
                                    echo '</div>';
                                }
...
//                        document.getElementById('result').innerHTML = "No barcode found";
     ...
     <script>
         function add_subtable(sub_table,table_id){
             $('#table_block-'+sub_table+'-'+table_id).show();
         }       
             
         function delete_subtable(sub_table,table_id){
              $('#table_block-'+sub_table+'-'+table_id).hide();
         }    
