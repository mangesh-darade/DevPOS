                                ...
                                <div id="print">
                                    <div id="left-middle">
                                        <div id="product-list" style="padding-bottom: 3em !important;">
                                            ...
                                            <!-- Left Thead -->
                                            <table class="table items table-striped table-bordered table-condensed table-hover" id="posTable" style="margin-bottom: 50;">
                                                <thead>
                                                    <tr>
                                        ...
                                        <div id="left-bottom">
                                            ...
                                            <div id="botbuttons" class="col-xs-12 text-center">
                                                ...
                                                <?php  if($pos_settings->supercash_amount == 1) { ?>
                                                    ...
                                                    <div class="col-xs-6 padding0" style="padding:0 10px 0 0;" id="">
                                                        <div class="" id="super-cash-id" style="padding: 7px 0px 5px 0px;background-color: #54b2ce; color: white;">
                                                            <input type="checkbox"  id="super-cash-id1" name="supercash" class="super-cash" value="1"  ><span style="padding:0 6px 0 6px;margin:3px 0 0 0;" id="display-supercash">Super Cash</span>
                                                        </div>
                                                    </div>
                                                    <div class="col-xs-6" style="padding:0 10px 0 0; display:none;" id="discription_input_block" >

                                                        <div class="btn-group-vertical btn-block">
                                                            <input type="text" name="pos_note" id="note" class="form-control" placeholder="Description" autocomplete="off">
                                                        </div>
                                                    </div>
                                                    <div class="col-xs-6 padding0" style="padding:0 10px 0 0;" id="discription_btn_block">
                                                        <div class="btn-group-vertical btn-block">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-sm-7">
                                                                <div class="input-group">
                                                                    ...
                                                                    <input type="text" placeholder="Customer Name"  name="customer_name" class="form-control kb-text txt-box ui-keyboard-input ui-widget-content ui-corner-all ui-keyboard-autoaccepted customer_name" id="customer_name" class="customer_name" value="<?= ($_SESSION['quick_customername'] ) ? $_SESSION['quick_customername'] . '(' . $_SESSION['quick_customerphone'] . ')' : $get_cmp->name; ?>"/>
                        ...
                        <form class="form-horizontal" role="form">
                            <p id="returnmessage"></p>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8" id="product_tax_supercash1">
                                        <?php
                                        $tr[""] = "";
...
?>
    ...
    $('#payment').click(function () {
            ...
            if ($("#super-cash-id1").prop('checked')==true){ 
                $('#checkbox_5').trigger('click');
             }
            checkoutOffers();
            <?php if ($sid) { ?>
                suspend = $('<span></span>');
...
<?php }//end else     ?>
            ...
            // totaldeposit()
            depositGiftAmt();
            if($("#super-cash-id1").is(":checked") == true)
            {
                $('#paid_by_1').val('supercash').trigger('change');
                $('[type="radio"][value="supercash"]').prop( "checked", true );
            }
            dueamount();
//                    setTimeout(function(){                       
...
//                    }, 1000);
         ...
         //supercash amount finalize screen    showtotalamount
        
        function supercashAmt() {
            var cusotmer_id = $('#poscustomer').val();
            $.ajax({
            type: 'ajax',
                    dataType: 'json',
                    method: 'get',
                    url: '<?= base_url('customers/supercashAmt/') ?>' + cusotmer_id,

                    async:true,
                    success: function (result) {
                    console.log('success');
                    if (result.supercash) {
                    if (result.supercash > 0){

                    $('input[value="supercash"]').trigger('click');
                    }
                    $('#showsupercash').show();
                    $('#showtotalamount').show();
                    $('#showsupercash').html('Supercash Amount: Rs. ' + result.supercash);
                    $('#showtotalamount').html('Total Deposit Amount: Rs. ' + (result.deposit_amount));
                    }
                    if (result.supercash) {
                    $('#showamtbalance').show();
                    }
            ...
            if (select_payed_type == 'supercash') {
            $('.final-submit-btn').prop('disabled', false);
            $('#errorsupercash_1').html('');
            setCustomerSupercash(amt, 'samount');
            }
...
<?php } ?>
            ...
            // Supercash Form_group Class ID
            if (p_val == 'supercash') {
            $('.final-submit-btn').prop('disabled', true);
            $('.sc_' + pa_no).show();
            } else {
            $('.sc_' + pa_no).hide();
            $('#supercashdetails_' + pa_no).html('');
            $('#errorsupercash_' + pa_no).html('');
            }
...
?>
        ...
        <script type="text/javascript">
            ...
            else if (custom_payment_value == 'supercash') {
            $('.final-submit-btn').prop('disabled', false);
            var bal = $('#amount_1').val();
            setCustomerSupercash(bal, 'supercash');
            }
            $('#amount_1').focus();
            });
            $('#add_item').blur();
...
//        alert(passurl);
            ...
            //supercash pos sale screen**
            function  setCustomerSupercash(ba, sel) {
                // alert(sel);
            var today = '<?= date('Y-m-d') ?>';
            var cu = $('#poscustomer').val();
            $.ajax({
            type: "get",
                    url: "<?= site_url('pos/searchSupercashByCustomer') ?>",
                    data: {customer_id: cu, bill_amt: ba},
                    dataType: "json",
                    success: function (data) {
                    ...
                    //$('#amount_1').val(ba).focus();
                    $('#supercashdetails_1').html('<small> Balance: ' + data.balanceamt + '</small>');
                    if (ba > parseFloat(data.balanceamt)) {
                    $('#errorsupercash_1').html('<small class="red">Amount Greater than Supercash</small>');
                    $('.final-submit-btn').prop('disabled', true);
                    if (sel == 'supercash') {
                    bootbox.alert('Invoice amount is greater that available Supercash balance please select other payment mode');
                    }

                    }
                    } else {
                    $('#supercashdetails_1').html('<small class="red"> Supercash not found for this customer</small>');
                    }
                    }
            });
            }
            ...
            type: "get",
                    ...
                    if (data.balanceamt > 0) {
                    //$('#amount_1').val(ba).focus();
                    $('#depositdetails_1').html('<small> Balance: ' + data.balanceamt + '</small>');
                    if (ba > parseFloat(data.balanceamt)) {
...
<?php if ($Settings->theme != 'default') { ?> 
            <script>
                ...
                type: 'get',
                        ...
                        //}else{  
                        if (data.name === C_name) {      
                        $('#super-cash-id1').prop('disabled', true);
                        } else {
                        $('#super-cash-id1').prop('disabled', false);
                        }
            ...
            <div class="page-break" id="deposit_print_bill" style="display:none" > 
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $this->sma->formatMoney($customerData->supercash_amount) ?></td>
                        <td><?= $this->sma->formatMoney(($customerData->deposit_amount?$customerData->deposit_amount:"0")+$customerData->supercash_amount) ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><?= $this->sma->formatMoney(($customerData->deposit_amount?$customerData->deposit_amount:"0")+$deposit_data['amount'] - $deposit_data['super_cash']) ?></td>
                        <td><?= $this->sma->formatMoney(($customerData->supercash_amount +$deposit_data['super_cash'])) ?></td>
                        <td><?= $this->sma->formatMoney((($customerData->deposit_amount?$customerData->deposit_amount:"0")+$deposit_data['amount'] - $deposit_data['super_cash'])+$customerData->supercash_amount +$deposit_data['super_cash']) ?></td>
                    </tr>
                    <tr>
                        <td > Paid By </td>
                        <td colspan="3"> <?= $deposit_data['paid_by'] ?> </td>
                    </tr>
                </tbody>
                 <?php } 
                 else { ?>
                 <tbody>
                    ...
                    <tr>
                        <!-- supercash Deposit Recharge Slip  -->
...
<?php } ?>
 ...
 <script>
             $(document).ready(function () {
              ...
              /*  $('.super-cash').change(function () {
                    var tax = $('#ptax').val();
                    var str = $('#titems').html();
                    // var  gtotal= $('#gtotal').html();
                    // const myArray = gtotal.split(".");
                    // // alert(parseInt(myArray[0]))

                    // const amt_array = myArray[0].split(",");
                   
                    // // alert(amt)

                    // var supercash = $('#display-supercash').text()
                    // const myArray1 = supercash.split("(");
                    // const myArray2 = myArray1[1].split(")");
                    // var su_amt = parseInt(myArray2[0])

                    // var a = ''
                    // var j=0;

                    //     for(j=0;j<amt_array.length;j++)
                    //     {
                    //         a = a+""+amt_array[j]
                    //     }
                    // var checkbox = $('#super-cash-id1');
                    // var amt = parseInt(a)
                    // if(amt >  su_amt)
                    // {
                    ...
                    //     checkbox.prop('checked', false).prop('enabled', true);
                    //     return
                    // }
                    var cnt = str.slice(0,1);
                    var i=0;
                    $.ajax({
                    type: 'ajax',
                        dataType: 'json',
                        method: 'get',
                        url: '<?= site_url('pos/getTax') ?>',
                        data: {'tax': tax},
                        async: false,
                        success: function (result) {
                        if (result) {
                            if (!$("#super-cash-id1").is(":checked")) {

                                if (confirm('Are you sure to add tax again?')) {
                                $('#product_tax_supercash1').text("");
                                $("#product_tax_supercash1").append(`<?php $tr[""] = "";foreach ($tax_rates as $tax) { $tr[$tax->id] = $tax->name;  } echo form_dropdown('ptax', $tr, "", 'id="ptax" class="form-control pos-input-tip" style="width:100%;"'); ?>`);
                                for(i=0 ;i<cnt;i++)
                                {
                                    $('.posdel').trigger('click');
                                    // $('#ptax').val(result.tax).trigger('change'); //editItem
                                }
                                // window.location.reload();
                               }
                               else
                                {
                                    $('#super-cash-id1').trigger('click');
                                }
                            ...
                            else
                            {
                                ...
                                if(true){
                                    alert('old_tax:'+$('#old_tax').val())
                                    $('#product_tax_supercash1').text("");
                                    $("#product_tax_supercash1").append(`<?php $tr1["1"] = "0% GST"; echo form_dropdown('ptax', $tr1, "", 'id="ptax" class="form-control pos-input-tip" style="width:100%;"'); ?>`);
                                    for(i=0 ;i<cnt;i++)
                                    {
                                        $('.edit').trigger('click');
         ...
         // $("#super-cash-id").find("checkbox").each(function(){
        ...
        <script>
            ...
            // $('#customer_name').val(jQuery('#poscustomer').val());
            var pass_data = $('#customer_name').val();
            var words = pass_data.split('(');
            var phone = words[1].split(')');
            var checkbox = $('#super-cash-id1');
            var mobile = phone[0];
            ...
            $.ajax({
                type: 'ajax',
                        dataType: 'json',
                        method: 'get',
                        url: '<?= site_url('pos/getSuperCash') ?>',
                        data: {'mobile_no': mobile},
                        async: false,
                        success: function (result) {
                        if (result.supercash != null ) {
                            // alert(result.supercash);
                            $('#display-supercash').text("Supercash(" + result.supercash + ")");
                       
                        } else {
                            alert("Invalid Supercash Amount");
                        }
                        if (result.supercash == 0 ) {
                            // alert("HI");
                            checkbox.prop('disabled', true);
