...
<div class="modal-dialog">
 ...
 <?php if($company->id != '1'){ ?>
            <h4 class="modal-title" id="myModalLabel"><?php echo lang('add_deposit') . " (" . $company->name . ")"; ?>
                <br/> Deposit : <?= $this->sma->formatMoney( $company->deposit_amount) ?>
              <?php  if($this->pos_settings->supercash_amount == 1)
              {?>
                        <br/> SuperCash : <?= $this->sma->formatMoney( $company->supercash_amount) ?>
                        <br/> Total Balance : <?= $this->sma->formatMoney( $company->deposit_amount + $company->supercash_amount ) ?>
             <?php }?>
        ...
        <div class="modal-body">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <div class="form-group" id="deposittype">
                        <?php echo lang('Deposit_Type','Deposit_Type'); ?>
                        <div class="controls">
                            <select class="form-control" name="deposit_type" id="deposit_type">
                                <option value="">Select Deposit type 
                            </option>
                                <?php 
                                 foreach ($depositTypes as $dpt) {
                                ?>
                                <option value="<?= $dpt->id ?>"> <?= $dpt->dp_type ?> </option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
       ...
       $('#servicescheck').on('ifChecked', function () {
        ...
        $.ajax({
                type: 'ajax',
                        dataType: 'json',
                        method: 'get',
                        url: '<?= site_url('customers/getSuperCashPercentage') ?>',
                        data: {'id': id},
                        async: false,
                        success: function (result) {
                        if (result.supercash_percentage != null) {
                            if(parseFloat(amt) >= parseFloat(cash_limit)){
                                let offer = result.supercash_percentage; 
                                // alert(result.supercash_percentage);           
                                if (offer.indexOf("%") !== -1) {
                                    let per = offer.split('%');
