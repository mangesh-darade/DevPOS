...
// Order discount calculation
                ...
                if (this.id == pr_tax) {
                    if (this.type == 1) {
                        if (item_tax_method == 0) {
                            pr_tax_val = formatDecimal((((unit_price) * parseFloat(this.rate)) / (100 + parseFloat(this.rate)))/1.05);
                            pr_tax_rate = formatDecimal(this.rate/1.05) + '%';
    ...
    $(document).on('click', '#editItem', function () {
        ...
        positems[item_id].row.fup = 1,
                ...
                positems[item_id].row.tax_rate = new_pr_tax,   // tax percentage id
...
//        console.log(sortedItems);
        ...
        $.each(sortedItems, function () {
            ...
            if( $('#super-cash-id1').prop('checked') == true ){                 
                //alert('Tax Method: '+item_tax_method);
                //alert('unit_price 1:'+unit_price);
				if (item_tax_method == 0) {
                    unit_price = formatDecimal((unit_price - pr_tax_val), 6);
                 }
				pr_tax_val  = 0; 
				pr_tax_rate = 0; 
				product_tax = 0;
            ...
            if (site.settings.tax1 == 1) {
                if( $('#super-cash-id1').prop('checked') == true ){ 
                    tr_html += '<input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="1"><input type="hidden" class="sproduct_tax" id="sproduct_tax_' + row_no + '" value="0.00">';
                } else {
       ...
       /* if (posdiscount = localStorage.getItem('posdiscount')) {
        ...
        // --------------------
                    ...
                    const supercash = $('#display-supercash').text();
                    const substring = '(';
                    if(supercash.includes(substring))
                    {
