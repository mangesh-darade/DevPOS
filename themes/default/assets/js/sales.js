...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.unit_price, item_qty = item.row.qty, item_aqty = item.row.quantity, 
            item_tax_method = item.row.tax_method, 
            item_option = item.row.option, item_code = item.row.code, item_serial = ((item.row.serial === null || item.row.serial === "" || item.row.serial == "undefined") ? '' : item.row.serial), item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
...
if(batch_id){
            ...
            tr_html += '<input name="tax_method[]" type="hidden" class="tax_method" value="' + item_tax_method + '">';
