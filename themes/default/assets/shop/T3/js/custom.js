  ...
  /*=================== QTY INPUT ===================*/
    ...
    $('.quantity').each(function() {
      ...
      var spinner = jQuery(this),
        ...
        btnUp = spinner.find('.quantity-up').attr('id', 'QtyUp-'+QtyId),
        btnDown = spinner.find('.quantity-down').attr('id', 'QtyDown-'+QtyId),
        min = input.attr('min'),
        max = input.attr('max');
      ...
      btnUp.on("click", function() {
        ...
        if(oldValue==max){
                loader('show');
		$('#StockMsg_'+QtyId).fadeIn('slow');
		$('#StockMsg_'+QtyId).delay(5000).fadeOut('slow');
                loader();
	}
        if (oldValue >= max) {
          var newVal = oldValue;
		...
		updateCartQty(newVal, UpQtyId);
      });

      btnDown.on("click", function() {
		  var DownQtyId = this.id;
		//console.log(DownQtyId);
        var oldValue = parseFloat(input.val());
        ...
        spinner.find("input").val(newVal);
        spinner.find("input").trigger("change");
		updateCartQty(newVal, DownQtyId);
      });
